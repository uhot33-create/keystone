...
Vercel + Neon（Postgres）で動作します。お散歩メモの画像は Vercel Blob に保存します（Neon には URL のみ）。Hobby / Neon Free を想定しています。メニュー下部の「今日は何の日」「今日の格言」「今日の小話」「今日の犬の豆知識」「今日の占い」は、Wikipedia・祝日API・名言API・朝日新聞デジタルなど公開Webからその日の内容を取得します。
